...
function SaitoTop(dir, callback) {
    ...
    function updateStat(stat) { // ಠ_ಠ
        ...
        if (saito.stats[event]) {
            ...
            for (const key in stat.stats) {
                const valueChanged = !old.stats[key] || old.stats[key].value !== stat.stats[key].value;
                let last_value = null;
                if (old.stats[key]) {
                    last_value = saito.stats[event].stats[key].value;
                }
                // Always update the stat to refresh the date, even if value hasn't changed
                saito.stats[event].stats[key] = stat.stats[key];
                // Only set last_value if the value actually changed
                if (valueChanged) {
                    saito.stats[event].stats[key].last_value = last_value;
                } else {
                    // Preserve existing last_value if value hasn't changed
                    saito.stats[event].stats[key].last_value = old.stats[key].last_value;
                }
            }
        } else {
    ...
    saito.config_file = path.join(saito.config_dir, "options");
    ...
    // Only read new lines, not existing ones
    ...
    tail.on("line", (line) => {
        ...
        try {
            ...
            // Skip invalid lines - uncomment to debug parsing issues
...
class SummaryComponent extends Component {
    ...
    render(data) {
        ...
        if (event = stats["mining::golden_tickets"]) {
            const stat = event["stats"] && event["stats"]["current target"];
            if (stat) {
        ...
        if (event = stats["mempool::state"]) {
            if (event["stats"] && event["stats"]["transactions"] && event["stats"]["blocks_queue"]) {
        ...
        if (event = stats["mining::golden_tickets"]) {
            const stat = event["stats"] && event["stats"]["miner_active"];
            if (stat) {
